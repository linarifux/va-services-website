...
import { createBlog, clearBlogState } from '../../redux/slices/blogSlice';
  ...
  return (
    <div className="flex flex-wrap gap-1 p-2 border-b border-stone-200 bg-stone-50 rounded-t-sm">
      <button
        ...
        title="Heading 2"
      >
        <span className="font-serif font-bold">H2</span>
      </button>
      <button
        type="button"
        ...
        title="Heading 3"
      >
        <span className="font-serif font-bold text-sm">H3</span>
      </button>
      ...
      <button
        ...
        title="Strikethrough"
      >
        <s className="font-serif text-sm">S</s>
      </button>
      ...
      <button
        ...
        title="Bullet List"
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16M8 6h.01M8 12h.01M8 18h.01"></path></svg>
      </button>
      <button
        type="button"
        ...
        title="Ordered List"
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 6h14M7 12h14M7 18h14M3 6h.01M3 12h.01M3 18h.01"></path></svg>
      </button>
      <button
        type="button"
      ...
      <button
        ...
        className={`p-2 rounded-sm transition-colors duration-200 focus:outline-none ${editor.isActive('link') ? 'text-stone-600 hover:bg-stone-200' : 'text-stone-300 cursor-not-allowed'}`}
        title="Remove Link"
      >
...
const AdminBlogs = () => {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Underline,
      Link.configure({
        openOnClick: false, // Prevents clicking the link while editing
      }),
    ],
    content: '<p>Start drafting your article here...</p>',
    editorProps: {
      attributes: {
        // Tailwind classes applied directly to the text area
  ...
  // Clean form and reset editor after success
  useEffect(() => {
    if (isSuccess) {
      setFormData({
        title: '',
      ...
      if (editor) {
        editor.commands.setContent('<p>Start drafting your article here...</p>');
      }
    }
  }, [isSuccess, user, editor]);
  ...
  const handleSubmit = (e) => {
    ...
    // Get the HTML content from Tiptap when the form is submitted
    ...
    const blogData = {
      ...formData,
      content: htmlContent, // Injecting the Tiptap HTML here
      tags: formattedTags,
    };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div>
        <h2 className="text-xl font-serif text-stone-900 mb-1">Publish New Article</h2>
        <p className="text-sm text-stone-500 font-sans">Draft and publish insights to the public blog using Tiptap.</p>
      </div>
      ...
      <div className="bg-white border border-stone-200 rounded-sm shadow-sm overflow-hidden p-6 sm:p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="e.g. Automating Shopify Workflows"
              />
            </div>
            
            <div className="space-y-2">
              <label htmlFor="slug" className="block text-sm font-sans font-medium text-stone-700">URL Slug (Auto-generated)</label>
              <input
                type="text"
                id="slug"
                name="slug"
                ...
                onChange={handleChange}
                required
                className="w-full px-4 py-3 bg-stone-100 border border-stone-200 text-stone-500 rounded-sm outline-none font-sans cursor-not-allowed"
                placeholder="e.g. automating-shopify-workflows"
                readOnly
              />
            </div>
          ...
          <div className="space-y-2">
            <label className="block text-sm font-sans font-medium text-stone-700">
              Article Content
            </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={handleChange}
                className="w-full px-4 py-3 bg-stone-50 border border-stone-200 focus:border-amber-700 focus:ring-2 focus:ring-amber-700/20 rounded-sm outline-none transition-all duration-300 font-sans text-stone-900"
                placeholder="E-commerce, Tech, Operations"
              />
            </div>
            ...
            <div className="flex items-center h-full pb-3">
              <input
                ...
                onChange={handleChange}
                className="w-5 h-5 text-amber-700 bg-stone-50 border-stone-300 rounded focus:ring-amber-700 accent-amber-700 cursor-pointer"
              />
              <label htmlFor="isPublished" className="ml-3 block text-sm font-sans font-medium text-stone-900 cursor-pointer">
                Publish immediately to public blog
              </label>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-stone-100 flex justify-end">
            <button
              type="submit"
              disabled={isLoading}
              className="w-full sm:w-auto px-8 py-3 bg-stone-900 text-stone-50 font-sans text-sm tracking-wider uppercase hover:bg-amber-700 disabled:bg-stone-400 disabled:cursor-not-allowed transition duration-300 rounded-sm flex justify-center items-center shadow-sm"
            >
              {isLoading ? (
